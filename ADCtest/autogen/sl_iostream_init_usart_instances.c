...
sl_status_t sl_iostream_usart_init_usart(void)
{
  ...
  init_usart.hwFlowControl = SL_IOSTREAM_USART_USART_FLOW_CONTROL_TYPE != uartFlowControlSoftware ? SL_IOSTREAM_USART_USART_FLOW_CONTROL_TYPE : usartHwFlowControlNone;
#endif
  ...
  sl_iostream_dma_config_t dma_config_usart = {.src = (uint8_t *)&SL_IOSTREAM_USART_USART_PERIPHERAL->RXDATA,
                                                        .peripheral_signal = SL_IOSTREAM_USART_RX_DMA_SIGNAL(SL_IOSTREAM_USART_USART_PERIPHERAL_NO)};

  sl_iostream_uart_config_t uart_config_usart = {
    .dma_cfg = dma_config_usart,
  ...
  uart_config_usart.sw_flow_control = SL_IOSTREAM_USART_USART_FLOW_CONTROL_TYPE == uartFlowControlSoftware;
...
void sl_iostream_usart_init_instances(void)
{
  ...
  status = sl_iostream_usart_init_usart();
  EFM_ASSERT(status == SL_STATUS_OK);
...
{
  sl_iostream_usart_irq_handler(&sl_iostream_usart);
}
                           ...
                           sl_power_manager_em_t to)
{
  ...
  if (to < SL_POWER_MANAGER_EM2){
    ...
    if (sl_iostream_uart_usart_handle->stream.context != NULL) {
      sl_iostream_uart_prepare_for_sleep(sl_iostream_uart_usart_handle);
    }
